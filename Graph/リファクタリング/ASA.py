...
class ASA(Device):
    ...
    #入力されたIPをデバイスのIFに設定されているか
    def is_ip_assigned(self, ip_address, netmask="32"):
        Flag = False
        for route in self.local_routes:
            if ipaddress.IPv4Network(route["NETWORK"] + "/" + route["NETMASK"]) == ipaddress.IPv4Network(ip_address + "/" + netmask):
                Flag = True
                break
        return Flag
    ...
    def find_nexthop(self, ip_address):
        ...
        for route in self.all_routes:
            network = ipaddress.IPv4Network(route["NETWORK"] + "/" + route["NETMASK"], strict=False)
        ...
        for prefix in range(32, -1, -1):
            for route in match_route:
                if route["NETMASK"] == self.prefix_to_subnet_mask(prefix):
