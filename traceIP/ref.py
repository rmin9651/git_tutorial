...
def trace_route(search_IP, device, route_type, path, result, TTL=255, NEXTHOP_IP=""):
    #pdb.set_trace()
    ...
    for next_hop in next_hops:
        ...
        trace_route(search_IP, next_device, next_route_type, path, result, TTL-1, NEXTHOP_IP="")
...
if not first_device:
    ...
    before_device_name, before_route_type = get_valid_device_name()

    # 最初のデバイスからルート情報の検索を開始
    next = search_route(search_IP, before_device_name, before_route_type)
    next_device_name, next_route_type = search_next_device(next[0]["NEXTHOP_IP"])
    ...
    for TTL in range(255):
        ...
        if next_device_name is None and next_route_type is None:
            ...
            break
        #ルート検索
        next = search_route(search_IP, next_device_name, next_route_type)
...
for next_hop in next_hops:
    next_device, next_route_type = search_next_device(next_hop["NEXTHOP_IP"])
    trace_route(dst_IP, next_device, next_route_type, [first_device], result)
