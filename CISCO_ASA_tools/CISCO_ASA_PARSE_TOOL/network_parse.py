...
def mask_or_prefix_to_prefix(input_value):
    if '/' in input_value:
        ...
        return subnet_mask_to_prefix(input_value)

def subnet_mask_to_prefix(subnet_mask):
    try:
        # サブネットマスクをIPv4Addressオブジェクトに変換
        subnet_mask = ipaddress.IPv4Address(subnet_mask)

        # サブネットマスクをビット列に変換
        subnet_mask_bits = bin(int(subnet_mask))[2:].zfill(32)

        # サブネットマスクが連続した1でない場合はエラーを発生させる
        if '01' in subnet_mask_bits:
            raise ValueError("Invalid subnet mask")

        # ビット列からプレフィックス長を計算
        prefix_length = subnet_mask_bits.count('1')

        # プレフィックスを返す
        return f'/{prefix_length}'

    except (ValueError, ipaddress.AddressValueError):
        # サブネットマスクが無効な場合はエラーメッセージを返す
...
def get_network(config = config_data):
    ntw = {"obj": {}, "grp": {}}
    parse_and_store_data(ntw_obj_template, ntw["obj"], config)
    parse_and_store_data(ntw_grp_template, ntw["grp"], config)
    # 入れ子構造の解消
