...
def process_service_data(data):
    ...
    elif data["PORT_OBJECT"]:
        dst_ope = "eq"
        dst_port = port_change(data["PORT_OBJECT"])
    elif data["PORT_OBJECT_START"] and data["PORT_OBJECT_END"]:
        dst_ope = "range"
...
def is_valid_port_protocol(input_str):
    ...
    pattern = r'^(\d{1,5}/(tcp|udp|tcp-udp))$'
...
def get_service():
    ...
    parse_and_store_data(srv_obj_template, srv["obj"], config_data)
    parse_and_store_data(srv_grp_template, srv["grp"], config_data)
