...
def get_access_list():
    ...
    access_list_data_src_network = dict()
    access_list_data_dst_network = dict()
    access_list_data_srv_network = dict()
    parse_and_store_data(access_list_template, access_list_data, config_data)
    for key, values in access_list_data.items():
        access_list_data_srv_network[key] = [process_access_list_data_service(value) for value in values]
    for key, values in access_list_data.items():
        access_list_data_src_network[key] = [process_access_list_data_network(value, src=True) for value in values]
    for key, values in access_list_data.items():
        access_list_data_dst_network[key] = [process_access_list_data_network(value, src=False) for value in values]

    return access_list_data_srv_network, access_list_data_src_network, access_list_data_dst_network
